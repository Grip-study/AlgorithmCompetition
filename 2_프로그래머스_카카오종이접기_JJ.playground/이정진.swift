...
func solution(_ n:Int) -> [Int] {
    guard (1...20) ~= n else { return [] }

    let foldCount = n
    var previousFoldedState = [PaperState]()
    var currentFoldedState = [PaperState]()

    for _ in 1...foldCount {
        currentFoldedState.removeAll()
        currentFoldedState.append(.foldedIn)

        for (index, state) in previousFoldedState.enumerated() {
            currentFoldedState.append(state)
            currentFoldedState.append(index % 2 == 0 ? .foldedOut : .foldedIn)
        }

        previousFoldedState = currentFoldedState
    }

    return currentFoldedState.map { $0.rawValue }
}
